...
const BEDTIME_ORDER = ["8-10pm", "10-12am", "12-2am", "2am-later", "depends"];
const WAKE_ORDER = ["5-7am", "7-9am", "9-11am", "12pm-later", "depends"];
const GUEST_ORDER = ["never", "rarely", "sometimes", "often", "very-often"];
const NOISE_ORDER = ["silent", "low", "moderate", "high", "any"];
const SPACE_ORDER = ["always-room", "mostly-room", "mix", "mostly-common", "always-common"];
const RELATIONSHIP_ORDER = ["keep-to-ourselves", "friendly-independent", "hang-out-sometimes", "good-friends", "close-friends"];
const CONFLICT_ORDER = ["direct", "hints", "let-go", "avoid", "depends"];
...
function ordinalSimilarity(a: string, b: string, order: string[]): number {
  ...
  // "depends" = flexible: treat as neutral (0.5) when either person chose it
  if (a === "depends" || b === "depends") return 0.5;
...
export function computeTraitScores(
  ...
  if (answersA.sleepBedtime && answersB.sleepBedtime) {
    scores.push({
      key: "sleepBedtime",
      similarity: ordinalSimilarity(
        answersA.sleepBedtime as string,
        answersB.sleepBedtime as string,
        BEDTIME_ORDER
      ),
    });
  }

  if (answersA.sleepWake && answersB.sleepWake) {
    scores.push({
      key: "sleepWake",
      similarity: ordinalSimilarity(
        answersA.sleepWake as string,
        answersB.sleepWake as string,
        WAKE_ORDER
      ),
    });
  }
  ...
  if (answersA.cleanliness != null && answersB.cleanliness != null) {
    const ca = Number(answersA.cleanliness);
    const cb = Number(answersB.cleanliness);
    if (!Number.isNaN(ca) && !Number.isNaN(cb)) {
      scores.push({
        key: "cleanliness",
        similarity: numericSimilarity(ca, cb, 5),
      });
    }
  }
  ...
  if (answersA.spaceUsage && answersB.spaceUsage) {
    scores.push({
      key: "spaceUsage",
      similarity: ordinalSimilarity(
        answersA.spaceUsage as string,
        answersB.spaceUsage as string,
        SPACE_ORDER
      ),
    });
  }

  if (answersA.roommateRelationship && answersB.roommateRelationship) {
    scores.push({
      key: "roommateRelationship",
      similarity: ordinalSimilarity(
        answersA.roommateRelationship as string,
        answersB.roommateRelationship as string,
        RELATIONSHIP_ORDER
      ),
    });
  }

  if (answersA.conflictStyle && answersB.conflictStyle) {
    scores.push({
      key: "conflictStyle",
      similarity: ordinalSimilarity(
        answersA.conflictStyle as string,
        answersB.conflictStyle as string,
        CONFLICT_ORDER
      ),
    });
  }
